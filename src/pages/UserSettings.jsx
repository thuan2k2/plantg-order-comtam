...
import UserAvatar from '../components/UserAvatar'; // <-- Nhúng Avatar có khung viền
import { getRankInfo } from '../utils/rankUtils'; // <-- Gọi hàm tính Rank
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 font-sans transition-colors duration-300 pb-20">
      <div className="max-w-md mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-8 shadow-xl border border-gray-100 dark:border-gray-700 animate-in fade-in duration-500">
          {/* MỚI: AVATAR SECTION SỬ DỤNG COMPONENT USERAVATAR */}
          <div className="flex flex-col items-center mb-10 relative">
            ...
            <div className="relative group z-10">
              {isLoadingRank ? (
                 <div className="w-28 h-28 bg-gray-200 dark:bg-gray-700 rounded-full animate-pulse border-4 border-white dark:border-gray-800"></div>
              ) : (
          ...
          {/* MỚI: KHỐI ĐẶC QUYỀN AUTO (GAMIFICATION) */}
          {rankInfo && (
            <div className="mb-10 p-5 bg-gray-50 dark:bg-gray-700/30 rounded-3xl border border-gray-100 dark:border-gray-700">
              ...
              <div className="space-y-2">
                <label className={`flex items-center p-3 rounded-2xl border-2 transition-all cursor-pointer ${activeAutoPerk === 'NONE' ? 'border-gray-800 dark:border-white bg-white dark:bg-gray-600 shadow-md' : 'border-transparent hover:bg-white dark:hover:bg-gray-800'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-3 ${activeAutoPerk === 'NONE' ? 'border-gray-800 dark:border-white' : 'border-gray-300'}`}>
                    {activeAutoPerk === 'NONE' && <div className="w-2 h-2 bg-gray-800 dark:bg-white rounded-full"></div>}
                  </div>
                  <span className="font-bold text-xs text-gray-700 dark:text-gray-200">Tắt Auto</span>
                </label>

                <label className={`flex items-center p-3 rounded-2xl border-2 transition-all ${rankInfo.current.autoPerks.includes('GIFT') || rankInfo.current.autoPerks.includes('ALL') ? 'cursor-pointer' : 'opacity-40 grayscale cursor-not-allowed'} ${activeAutoPerk === 'GIFT' ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/30 shadow-md' : 'border-transparent hover:bg-white dark:hover:bg-gray-800'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-3 ${activeAutoPerk === 'GIFT' ? 'border-orange-500' : 'border-gray-300'}`}>
                    {activeAutoPerk === 'GIFT' && <div className="w-2 h-2 bg-orange-500 rounded-full"></div>}
                  </div>
                  <div className="flex-1">
                    <p className="font-bold text-xs text-gray-800 dark:text-gray-200 flex justify-between">
                      Tự nhận Hộp Quà 🎁
                      {(!rankInfo.current.autoPerks.includes('GIFT') && !rankInfo.current.autoPerks.includes('ALL')) && <span className="text-[8px] text-gray-400">Từ Hạng Bạc</span>}
                    </p>
                  </div>
                </label>

                <label className={`flex items-center p-3 rounded-2xl border-2 transition-all ${rankInfo.current.autoPerks.includes('CHECKIN') || rankInfo.current.autoPerks.includes('ALL') ? 'cursor-pointer' : 'opacity-40 grayscale cursor-not-allowed'} ${activeAutoPerk === 'CHECKIN' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 shadow-md' : 'border-transparent hover:bg-white dark:hover:bg-gray-800'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-3 ${activeAutoPerk === 'CHECKIN' ? 'border-blue-500' : 'border-gray-300'}`}>
                    {activeAutoPerk === 'CHECKIN' && <div className="w-2 h-2 bg-blue-500 rounded-full"></div>}
                  </div>
                  <div className="flex-1">
                    <p className="font-bold text-xs text-gray-800 dark:text-gray-200 flex justify-between">
                      Tự Điểm danh 📅
                      {(!rankInfo.current.autoPerks.includes('CHECKIN') && !rankInfo.current.autoPerks.includes('ALL')) && <span className="text-[8px] text-gray-400">Từ Bạch Kim</span>}
                    </p>
                  </div>
                </label>

                <label className={`flex items-center p-3 rounded-2xl border-2 transition-all ${rankInfo.current.autoPerks.includes('LUCKY') || rankInfo.current.autoPerks.includes('ALL') ? 'cursor-pointer' : 'opacity-40 grayscale cursor-not-allowed'} ${activeAutoPerk === 'LUCKY' ? 'border-red-500 bg-red-50 dark:bg-red-900/30 shadow-md' : 'border-transparent hover:bg-white dark:hover:bg-gray-800'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-3 ${activeAutoPerk === 'LUCKY' ? 'border-red-500' : 'border-gray-300'}`}>
                    {activeAutoPerk === 'LUCKY' && <div className="w-2 h-2 bg-red-500 rounded-full"></div>}
                  </div>
                  <div className="flex-1">
                    <p className="font-bold text-xs text-gray-800 dark:text-gray-200 flex justify-between">
                      Tự mở Lì xì (Lucky) 🧧
                      {(!rankInfo.current.autoPerks.includes('LUCKY') && !rankInfo.current.autoPerks.includes('ALL')) && <span className="text-[8px] text-gray-400">Từ Tinh Anh</span>}
                    </p>
                  </div>
                </label>

                {rankInfo.current.autoPerks.includes('ALL') && (
                  <label className={`flex items-center p-3 rounded-2xl border-2 transition-all cursor-pointer ${activeAutoPerk === 'ALL' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/30 shadow-md shadow-purple-200 dark:shadow-none' : 'border-transparent hover:bg-white dark:hover:bg-gray-800'}`}>
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-3 ${activeAutoPerk === 'ALL' ? 'border-purple-500' : 'border-gray-300'}`}>
                      {activeAutoPerk === 'ALL' && <div className="w-2 h-2 bg-purple-500 rounded-full"></div>}
                    </div>
                    <div className="flex-1">
                      <p className="font-black text-xs text-purple-600 dark:text-purple-400">Vua Trò Chơi (Bật ALL) 👑</p>
                    </div>
                  </label>
                )}
              </div>
